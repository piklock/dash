    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "dash" is a composite category enabling all Owncoin-related debug output
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OwncoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OwncoinCore
    // Mac: ~/Library/Application Support/OwncoinCore
    // Unix: ~/.dashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OwncoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OwncoinCore";
#else
    // Unix
